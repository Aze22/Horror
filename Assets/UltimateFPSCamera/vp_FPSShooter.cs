...
public class vp_FPSShooter : vp_Component
{
	...
	protected new void Awake()
	{
		...
		/*GUIController.ChangeAmmo(m_AmmoCount);
		GUIController.ChangeAmmoMax(AmmoMaxCount);*/
	...
	protected new void Update()
	{
		...
		base.Update();
		Debug.DrawRay(transform.FindChild("BulletOrigin").position, Camera.main.ScreenToWorldPoint(new Vector3(Screen.width*0.5f, Screen.height*0.5f,500000)));
	}
	...
	public void Fire()
	{
		if(canReload) {
			...
			if (StatsController._ammoInCurrentMag < 1)
			{
				DryFire();
				return;
			}
			...
			m_AmmoCount--;
			//GUIController.ChangeAmmo(m_AmmoCount);
			StatsController._ammoInCurrentMag--;
			if(StatsController._ammoInCurrentMag < 0) StatsController._ammoInCurrentMag = 0;
		...
		if(Physics.Raycast(transform.FindChild("BulletOrigin").position, Camera.main.ScreenToWorldPoint(new Vector3(Screen.width*0.5f, Screen.height*0.5f,500000)), out hitInfo, 500, StatsController.currentWeapon.shootLayerMask)) {
		
			if(hitInfo.collider.tag == "Zombie")
					hitInfo.collider.SendMessage("Hurt", SendMessageOptions.DontRequireReceiver);
	...
	public void Reload(int ammoCount)
	{
		...
		if (Time.time < m_NextAllowedReloadTime || !canReload)
			return;

		m_NextAllowedReloadTime = Time.time + AmmoReloadTime;
		
		
		m_AmmoCount = StatsController.currentWeapon.magCapacity;
		StatsController.RemoveFromInventory(InteractionController.InteractionType.Ammo, 1);
		
		GUIController.ChangeMagsAvailable();
		
		Invoke("UpdateAmmoGUI", AmmoReloadTime + 0.5f);
	...
	private void UpdateAmmoGUI() {
		...
		StatsController._ammoInCurrentMag = StatsController._magCapacity;
		m_AmmoCount = StatsController._magCapacity;
		canReload = true;
