...
public class vp_FPSPlayer : MonoBehaviour
{
	...
	void Start()
	{
		movementThreshold.x = Screen.width * 0.03f;
		movementThreshold.y = Screen.height * 0.03f;
	...
	public void SetMovementInput(Vector2 movementVector) {
		
		//Controller.MotorAcceleration
		//Debug.Log(x.ToString() + " , " + y.ToString());
		//Debug.Log(movementVector.ToString());
		
		if((movementVector.x == 0 && movementVector.y == 0)) return;
		
		if(movementVector.x > movementThreshold.x) Controller.MoveRight();
		else if(movementVector.x < -movementThreshold.x) Controller.MoveLeft();
		
		if(movementVector.y < -movementThreshold.y) Controller.MoveBack();
		else if(movementVector.y > movementThreshold.y) Controller.MoveForward();
		
		Controller.MotorAcceleration = (movementVector.magnitude * 0.2f) * Time.deltaTime;
		Controller.MotorAcceleration = Mathf.Clamp(Controller.MotorAcceleration, -0.16f, 0.16f);
	...
	protected void InputWalk()
	{
		...
		if(!TouchControls.touchControlsEnabled) {
			if (Input.GetButton("Forward")) { Controller.MoveForward(); }
			if (Input.GetButton("Left")) { Controller.MoveLeft(); }
			if (Input.GetButton("Back")) { Controller.MoveBack(); }
			if (Input.GetButton("Right")) { Controller.MoveRight(); }
		}
	...
	protected void InputFire()
	{
			...
			if ((Input.GetButton("Fire") && !TouchControls.touchControlsEnabled) || (TouchControls.touchControlsEnabled && TouchControls.shootTouchAsked))
			{
				TouchControls.shootTouchAsked = false;
	...
	protected void InputReload()
	{
		...
		if ((Input.GetButtonDown("Reload") && !TouchControls.touchControlsEnabled) || TouchControls.touchReloadAsked)
		{
			TouchControls.touchReloadAsked = false;
			if(CurrentShooter != null) {
				GUIController.ShowMags(true);
				if(StatsController._magsAvailable > 0 && StatsController._ammoInCurrentMag < StatsController._magCapacity) {
					
					
					Reload(StatsController.currentWeapon.magCapacity);
